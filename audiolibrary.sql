...
CREATE TABLE `Music artist/band's song`(song int, artband smallint, feat varchar(200) NULL, FOREIGN KEY (song) REFERENCES Song(ID), FOREIGN KEY (artband) REFERENCES `Music artist/band identifier`(ID));
...
CREATE VIEW TotalArtistsStatisticsVIEW AS SELECT REPLACE(REPLACE(REPLACE(CONCAT('Total: ', (SELECT SUM(noorigartsbands) FROM `Audio library`)-(SELECT COUNT(ID) FROM `Related music artist/band`), ', ', (SELECT SUM(nosongs) FROM `Audio library` WHERE cat!='Саундтреки')-(SELECT SUM(nosongs) FROM `Related music artist/band`), '/', FLOOR(((SELECT SUM(TIME_TO_SEC(dur)) FROM Song)-(SELECT SUM(TIME_TO_SEC(songsdur)) FROM `Related music artist/band`))/86400), 'd ', FLOOR((((SELECT SUM(TIME_TO_SEC(dur)) FROM Song)-(SELECT SUM(TIME_TO_SEC(songsdur)) FROM `Related music artist/band`))%86400)/3600), ':', FLOOR(((((SELECT SUM(TIME_TO_SEC(dur)) FROM Song)-(SELECT SUM(TIME_TO_SEC(songsdur)) FROM `Related music artist/band`))%86400)%3600)/60), ':', FLOOR(((((SELECT SUM(TIME_TO_SEC(dur)) FROM Song)-(SELECT SUM(TIME_TO_SEC(songsdur)) FROM `Related music artist/band`))%86400)%3600)%60), '; ', 'norelarts', ', ', SUM(`Music artist/band`.totnosongs)-SUM(`Music artist/band`.nosongs), '/', FLOOR((SUM(TIME_TO_SEC(`Music artist/band`.totsongsdur))-SUM(TIME_TO_SEC(`Music artist/band`.songsdur)))/86400), 'd ', FLOOR(((SUM(TIME_TO_SEC(`Music artist/band`.totsongsdur))-SUM(TIME_TO_SEC(`Music artist/band`.songsdur)))%86400)/3600), ':', FLOOR((((SUM(TIME_TO_SEC(`Music artist/band`.totsongsdur))-SUM(TIME_TO_SEC(`Music artist/band`.songsdur)))%86400)%3600)/60), ':', FLOOR((((SUM(TIME_TO_SEC(`Music artist/band`.totsongsdur))-SUM(TIME_TO_SEC(`Music artist/band`.songsdur)))%86400)%3600)%60)), "; 0, 0/0d 0:0:0", ""), "0d ", ""), "norelarts", (SELECT SUM(noartsbands) FROM Genre)-(SELECT COUNT(ID) FROM `Music artist/band` WHERE cat='Жанр')) AS 'Number of artists and related artists, songs count/duration' FROM `Music artist/band` UNION (SELECT REPLACE(REPLACE(REPLACE(CONCAT('Жанр: ', COUNT(`Music artist/band`.ID), ', ', SUM(`Music artist/band`.nosongs), '/', FLOOR(SUM(TIME_TO_SEC(`Music artist/band`.songsdur))/86400), 'd ', FLOOR((SUM(TIME_TO_SEC(`Music artist/band`.songsdur))%86400)/3600), ':', FLOOR(((SUM(TIME_TO_SEC(`Music artist/band`.songsdur))%86400)%3600)/60), ':', FLOOR(((SUM(TIME_TO_SEC(`Music artist/band`.songsdur))%86400)%3600)%60), '; ', 'norelarts', ', ', SUM(`Music artist/band`.totnosongs)-SUM(`Music artist/band`.nosongs), '/', FLOOR((SUM(TIME_TO_SEC(`Music artist/band`.totsongsdur))-SUM(TIME_TO_SEC(`Music artist/band`.songsdur)))/86400), 'd ', FLOOR(((SUM(TIME_TO_SEC(`Music artist/band`.totsongsdur))-SUM(TIME_TO_SEC(`Music artist/band`.songsdur)))%86400)/3600), ':', FLOOR((((SUM(TIME_TO_SEC(`Music artist/band`.totsongsdur))-SUM(TIME_TO_SEC(`Music artist/band`.songsdur)))%86400)%3600)/60), ':', FLOOR((((SUM(TIME_TO_SEC(`Music artist/band`.totsongsdur))-SUM(TIME_TO_SEC(`Music artist/band`.songsdur)))%86400)%3600)%60)), "; 0, 0/0d 0:0:0", ""), "0d ", ""), "norelarts", (SELECT SUM(noartsbands) FROM Genre)-(SELECT COUNT(ID) FROM `Music artist/band` WHERE cat='Жанр')) AS 'Number of artists and related artists, songs count/duration' FROM `Music artist/band` WHERE `Music artist/band`.cat='Жанр') UNION (SELECT REPLACE(REPLACE(CONCAT(`Music artist/band`.genr, ': ', COUNT(`Music artist/band`.ID), ', ', SUM(`Music artist/band`.nosongs), '/', FLOOR(SUM(TIME_TO_SEC(`Music artist/band`.songsdur))/86400), 'd ', FLOOR((SUM(TIME_TO_SEC(`Music artist/band`.songsdur))%86400)/3600), ':', FLOOR(((SUM(TIME_TO_SEC(`Music artist/band`.songsdur))%86400)%3600)/60), ':', FLOOR(((SUM(TIME_TO_SEC(`Music artist/band`.songsdur))%86400)%3600)%60), '; ', Genre.noartsbands-COUNT(`Music artist/band`.ID), ', ', SUM(`Music artist/band`.totnosongs)-SUM(`Music artist/band`.nosongs), '/', FLOOR((SUM(TIME_TO_SEC(`Music artist/band`.totsongsdur))-SUM(TIME_TO_SEC(`Music artist/band`.songsdur)))/86400), 'd ', FLOOR(((SUM(TIME_TO_SEC(`Music artist/band`.totsongsdur))-SUM(TIME_TO_SEC(`Music artist/band`.songsdur)))%86400)/3600), ':', FLOOR((((SUM(TIME_TO_SEC(`Music artist/band`.totsongsdur))-SUM(TIME_TO_SEC(`Music artist/band`.songsdur)))%86400)%3600)/60), ':', FLOOR((((SUM(TIME_TO_SEC(`Music artist/band`.totsongsdur))-SUM(TIME_TO_SEC(`Music artist/band`.songsdur)))%86400)%3600)%60)), "; 0, 0/0d 0:0:0", ""), "0d ", "") AS 'Number of artists and related artists, songs count/duration' FROM `Music artist/band` JOIN Genre ON Genre.nam=`Music artist/band`.genr GROUP BY Genre.nam ORDER BY Genre.nam) UNION (WITH artistsstatisticscovers AS(SELECT CONCAT('Каверы: ', COUNT(DISTINCT `Music artist/band's song`.artband), ', ', COUNT(Song.nam), '/', FLOOR(SUM(TIME_TO_SEC(Song.dur))/86400), 'd ', FLOOR((SUM(TIME_TO_SEC(Song.dur))%86400)/3600), ':', FLOOR(((SUM(TIME_TO_SEC(Song.dur))%86400)%3600)/60), ':', FLOOR(((SUM(TIME_TO_SEC(Song.dur))%86400)%3600)%60)) AS 'Number of artists and related artists, songs count/duration' FROM `Music artist/band's song` JOIN `Music artist/band` ON `Music artist/band`.ID=`Music artist/band's song`.artband JOIN Song ON Song.ID=`Music artist/band's song`.song WHERE Song.cat='Каверы' UNION (SELECT REPLACE(CONCAT(COUNT(DISTINCT `Music artist/band's song`.artband), ', ', COUNT(Song.nam), '/', FLOOR(SUM(TIME_TO_SEC(Song.dur))/86400), 'd ', FLOOR((SUM(TIME_TO_SEC(Song.dur))%86400)/3600), ':', FLOOR(((SUM(TIME_TO_SEC(Song.dur))%86400)%3600)/60), ':', FLOOR(((SUM(TIME_TO_SEC(Song.dur))%86400)%3600)%60)), "0d ", "") AS 'Number of artists and related artists, songs count/duration' FROM `Music artist/band's song` JOIN `Related music artist/band` ON `Related music artist/band`.ID=`Music artist/band's song`.artband JOIN Song ON Song.ID=`Music artist/band's song`.song WHERE Song.cat='Каверы' ORDER BY COUNT(song))) SELECT GROUP_CONCAT(`Number of artists and related artists, songs count/duration` SEPARATOR '; ') FROM artistsstatisticscovers);
CREATE VIEW TotalSongsStatisticsVIEW AS WITH songsstatisticsasc AS(WITH songsstatisticsasc AS(SELECT `Music artist/band`.artband, `Music artist/band's song`.feat, Song.nam, `Cover's original music artist/band`.artband AS coverorigartbandID, `Cover's original music artist/band`.feat AS coverorigfeat, Song.dur FROM `Music artist/band` JOIN `Music artist/band's song` ON `Music artist/band`.ID=`Music artist/band's song`.artband JOIN Song ON Song.ID=`Music artist/band's song`.song LEFT JOIN `Cover's original music artist/band` ON Song.ID=`Cover's original music artist/band`.song UNION SELECT `Related music artist/band`.relartband, `Music artist/band's song`.feat, Song.nam, `Cover's original music artist/band`.artband AS coverorigartbandID, `Cover's original music artist/band`.feat AS coverorigfeat, Song.dur FROM `Related music artist/band` JOIN `Music artist/band's song` ON `Related music artist/band`.ID=`Music artist/band's song`.artband JOIN Song ON Song.ID=`Music artist/band's song`.song LEFT JOIN `Cover's original music artist/band` ON Song.ID=`Cover's original music artist/band`.song) SELECT songsstatisticsasc.artband, songsstatisticsasc.feat, songsstatisticsasc.nam, `Music artist/band`.artband AS coverorigartband, coverorigfeat, songsstatisticsasc.dur FROM songsstatisticsasc LEFT JOIN `Music artist/band` ON `Music artist/band`.ID=songsstatisticsasc.coverorigartbandID) SELECT REPLACE(REPLACE(REPLACE(CONCAT(songsstatisticsasc.artband, REGEXP_REPLACE(REGEXP_REPLACE(REGEXP_REPLACE(IFNULL(songsstatisticsasc.feat, ''), '^', ', '), ',(?!.*,)', ' &'), '(?=.*[А-Яа-я])&', 'и'), ' - ', songsstatisticsasc.nam, ' (', IFNULL(songsstatisticsasc.coverorigartband, ''), ' & ', IFNULL(songsstatisticsasc.coverorigfeat, ''), ') ', '(', TRIM(LEADING '00:' FROM REPLACE(songsstatisticsasc.dur, ':0', ':')), ')'), ',  -', ' -'), ' ( & )', ''), ' & )', ')') AS 'Shortest songs', REPLACE(REPLACE(REPLACE(CONCAT(songsstatisticsdesc.artband, REGEXP_REPLACE(REGEXP_REPLACE(REGEXP_REPLACE(IFNULL(songsstatisticsdesc.feat, ''), '^', ', '), ',(?!.*,)', ' &'), '(?=.*[А-Яа-я])&', 'и'), ' - ', songsstatisticsdesc.nam, ' (', IFNULL(songsstatisticsdesc.coverorigartband, ''), ' & ', IFNULL(songsstatisticsdesc.coverorigfeat, ''), ') ', '(', TRIM(LEADING '00:' FROM REPLACE(songsstatisticsdesc.dur, ':0', ':')), ')'), ',  -', ' -'), ' ( & )', ''), ' & )', ')') AS 'Longest songs' FROM (SELECT *, ROW_NUMBER() OVER(ORDER BY dur, nam) AS rnasc FROM songsstatisticsasc) AS songsstatisticsasc JOIN (SELECT *, ROW_NUMBER() OVER(ORDER BY dur DESC, nam ASC) AS rndesc FROM songsstatisticsasc) AS songsstatisticsdesc ON songsstatisticsasc.rnasc=songsstatisticsdesc.rndesc;
CREATE VIEW NumberofMusicArtistsBandsInCategoryByNumberofSongsVIEW AS WITH nomusicartistsbandsincategorybynosongs AS(SELECT COALESCE(`Music artist/band`.ID, `Related music artist/band`.ID) AS artbandid, COALESCE(`Music artist/band`.artband, `Related music artist/band`.relartband) AS artband, IFNULL(IFNULL(genr, (SELECT genr FROM `Music artist/band` WHERE ID=(SELECT DISTINCT artband FROM `Related music artist/band` WHERE ID=artbandid))), (SELECT cat FROM `Music artist/band` WHERE ID=artbandid)) AS categor, COUNT(`Music artist/band's song`.song) AS nosongs FROM `Music artist/band's song` LEFT JOIN `Music artist/band` ON `Music artist/band`.ID=`Music artist/band's song`.artband LEFT JOIN `Related music artist/band` ON `Related music artist/band`.ID=`Music artist/band's song`.artband GROUP BY artbandid, artband, categor) SELECT nosongs AS 'Number of songs', categor AS Category, COUNT(artband) AS 'Number of music artists/bands' FROM nomusicartistsbandsincategorybynosongs GROUP BY nosongs, categor ORDER BY nosongs, categor;
CREATE VIEW MusicArtistsBandsInCategoryByNumberofSongsVIEW AS WITH musicartistsbandsincategorybynosongs AS(SELECT COALESCE(`Music artist/band`.ID, `Related music artist/band`.ID) AS artbandid, COALESCE(`Music artist/band`.artband, `Related music artist/band`.relartband) AS artband, IFNULL(IFNULL(genr, (SELECT genr FROM `Music artist/band` WHERE ID=(SELECT DISTINCT artband FROM `Related music artist/band` WHERE ID=artbandid))), (SELECT cat FROM `Music artist/band` WHERE ID=artbandid)) AS categor, COUNT(`Music artist/band's song`.song) AS nosongs FROM `Music artist/band's song` LEFT JOIN `Music artist/band` ON `Music artist/band`.ID=`Music artist/band's song`.artband LEFT JOIN `Related music artist/band` ON `Related music artist/band`.ID=`Music artist/band's song`.artband GROUP BY artbandid, artband, categor) SELECT artband AS 'Music artist/band', categor AS Category, nosongs AS 'Number of songs' FROM musicartistsbandsincategorybynosongs GROUP BY artband, nosongs, categor ORDER BY categor, nosongs, artband;
CREATE VIEW NumberofSongsByDurationRangeVIEW AS SELECT FLOOR(TIME_TO_SEC(dur)/60) AS 'Song duration range in minutes', COUNT(dur) AS 'Number of songs' FROM Song GROUP BY `Song duration range in minutes` ORDER BY `Song duration range in minutes`;
CREATE VIEW SongsByDurationRangeVIEW AS WITH songsbydurationrange AS(WITH songsbydurationrange AS(SELECT `Music artist/band`.artband, `Music artist/band's song`.feat, Song.nam, `Cover's original music artist/band`.artband AS coverorigartbandID, `Cover's original music artist/band`.feat AS coverorigfeat, Song.dur, FLOOR(TIME_TO_SEC(dur)/60) AS durrang FROM `Music artist/band` JOIN `Music artist/band's song` ON `Music artist/band`.ID=`Music artist/band's song`.artband JOIN Song ON Song.ID=`Music artist/band's song`.song LEFT JOIN `Cover's original music artist/band` ON Song.ID=`Cover's original music artist/band`.song UNION SELECT `Related music artist/band`.relartband, `Music artist/band's song`.feat, Song.nam, `Cover's original music artist/band`.artband AS coverorigartbandID, `Cover's original music artist/band`.feat AS coverorigfeat, Song.dur, FLOOR(TIME_TO_SEC(dur)/60) AS durrang FROM `Related music artist/band` JOIN `Music artist/band's song` ON `Related music artist/band`.ID=`Music artist/band's song`.artband JOIN Song ON Song.ID=`Music artist/band's song`.song LEFT JOIN `Cover's original music artist/band` ON Song.ID=`Cover's original music artist/band`.song) SELECT songsbydurationrange.artband, songsbydurationrange.feat, songsbydurationrange.nam, `Music artist/band`.artband AS coverorigartband, coverorigfeat, songsbydurationrange.dur, songsbydurationrange.durrang, NumberofSongsByDurationRangeVIEW.`Number of songs` AS nosongs FROM songsbydurationrange LEFT JOIN `Music artist/band` ON `Music artist/band`.ID=songsbydurationrange.coverorigartbandID JOIN NumberofSongsByDurationRangeVIEW ON songsbydurationrange.durrang=NumberofSongsByDurationRangeVIEW.`Song duration range in minutes` ORDER BY songsbydurationrange.nam, songsbydurationrange.durrang) SELECT REPLACE(REPLACE(REPLACE(CONCAT(songsbydurationrange.artband, REGEXP_REPLACE(REGEXP_REPLACE(REGEXP_REPLACE(IFNULL(songsbydurationrange.feat, ''), '^', ', '), ',(?!.*,)', ' &'), '(?=.*[А-Яа-я])&', 'и'), ' - ', songsbydurationrange.nam, ' (', IFNULL(songsbydurationrange.coverorigartband, ''), ' & ', IFNULL(songsbydurationrange.coverorigfeat, ''), ') ', '(', TRIM(LEADING '00:' FROM REPLACE(songsbydurationrange.dur, ':0', ':')), ')'), ',  -', ' -'), ' ( & )', ''), ' & )', ')') AS Song, songsbydurationrange.durrang AS 'Song duration range in minutes', songsbydurationrange.nosongs AS 'Number of songs' FROM songsbydurationrange;
...
WITH songsstatisticsasc AS(WITH songsstatisticsasc AS(SELECT `Music artist/band`.artband, `Music artist/band's song`.feat, Song.nam, `Cover's original music artist/band`.artband AS coverorigartbandID, `Cover's original music artist/band`.feat AS coverorigfeat, Song.dur FROM `Music artist/band` JOIN `Music artist/band's song` ON `Music artist/band`.ID=`Music artist/band's song`.artband JOIN Song ON Song.ID=`Music artist/band's song`.song JOIN `Cover's original music artist/band` ON Song.ID=`Cover's original music artist/band`.song WHERE Song.cat='Каверы' UNION SELECT `Related music artist/band`.relartband, `Music artist/band's song`.feat, Song.nam, `Cover's original music artist/band`.artband AS coverorigartbandID, `Cover's original music artist/band`.feat AS coverorigfeat, Song.dur FROM `Related music artist/band` JOIN `Music artist/band` ON `Music artist/band`.ID=`Related music artist/band`.artband JOIN `Music artist/band's song` ON `Related music artist/band`.ID=`Music artist/band's song`.artband JOIN Song ON Song.ID=`Music artist/band's song`.song JOIN `Cover's original music artist/band` ON Song.ID=`Cover's original music artist/band`.song WHERE Song.cat='Каверы') SELECT songsstatisticsasc.artband, songsstatisticsasc.feat, songsstatisticsasc.nam, `Music artist/band`.artband AS coverorigartband, coverorigfeat, songsstatisticsasc.dur FROM songsstatisticsasc JOIN `Music artist/band` ON `Music artist/band`.ID=songsstatisticsasc.coverorigartbandID) SELECT REPLACE(REPLACE(CONCAT(songsstatisticsasc.artband, REGEXP_REPLACE(REGEXP_REPLACE(REGEXP_REPLACE(IFNULL(songsstatisticsasc.feat, ''), '^', ', '), ',(?!.*,)', ' &'), '(?=.*[А-Яа-я])&', 'и'), ' - ', songsstatisticsasc.nam, ' (', IFNULL(songsstatisticsasc.coverorigartband, ''), ' & ', IFNULL(songsstatisticsasc.coverorigfeat, ''), ') ', '(', TRIM(LEADING '00:' FROM REPLACE(songsstatisticsasc.dur, ':0', ':')), ')'), ',  -', ' -'), ' & )', ')') AS 'Shortest songs', REPLACE(REPLACE(CONCAT(songsstatisticsdesc.artband, REGEXP_REPLACE(REGEXP_REPLACE(REGEXP_REPLACE(IFNULL(songsstatisticsdesc.feat, ''), '^', ', '), ',(?!.*,)', ' &'), '(?=.*[А-Яа-я])&', 'и'), ' - ', songsstatisticsdesc.nam, ' (', IFNULL(songsstatisticsdesc.coverorigartband, ''), ' & ', IFNULL(songsstatisticsdesc.coverorigfeat, ''), ') ', '(', TRIM(LEADING '00:' FROM REPLACE(songsstatisticsdesc.dur, ':0', ':')), ')'), ',  -', ' -'), ' & )', ')') AS 'Longest songs' FROM (SELECT *, ROW_NUMBER() OVER(ORDER BY dur, nam) AS rnasc FROM songsstatisticsasc) AS songsstatisticsasc JOIN (SELECT *, ROW_NUMBER() OVER(ORDER BY dur DESC, nam ASC) AS rndesc FROM songsstatisticsasc) AS songsstatisticsdesc ON songsstatisticsasc.rnasc=songsstatisticsdesc.rndesc;
