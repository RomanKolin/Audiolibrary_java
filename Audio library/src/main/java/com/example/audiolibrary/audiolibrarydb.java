    ...
    public static String[][] SELECTSEARCHING(int tabl, int obslist, String search) throws Exception
    {
        ...
        switch (tabl)
        {
            ...
            case 6:
                ...
                break;
            case 7:
                pstat = conn.prepareStatement("WITH soundtrack AS(WITH soundtrack AS((SELECT movanimsergam, artband, song, nosongs, REGEXP_REPLACE(TRIM(LEADING '0' FROM TRIM(LEADING '00:' FROM REPLACE(Song.dur, ':0', ':'))), '^:', '0:') AS songsdur, ID FROM Soundtrack WHERE movanimsergam LIKE ? ORDER BY movanimsergam) UNION (SELECT movanimsergam, artband, song, nosongs, REGEXP_REPLACE(TRIM(LEADING '0' FROM TRIM(LEADING '00:' FROM REPLACE(Song.dur, ':0', ':'))), '^:', '0:') AS songsdur, ID FROM Soundtrack WHERE artband LIKE ? ORDER BY movanimsergam) UNION (SELECT movanimsergam, artband, song, nosongs, REGEXP_REPLACE(TRIM(LEADING '0' FROM TRIM(LEADING '00:' FROM REPLACE(Song.dur, ':0', ':'))), '^:', '0:') AS songsdur, ID FROM Soundtrack WHERE song LIKE ? ORDER BY movanimsergam)) SELECT movanimsergam, artband, song, nosongs, songsdur, ID FROM soundtrack ORDER BY movanimsergam) SELECT COUNT(ID) FROM soundtrack;");
                pstat.setString(1, search);
                pstat.setString(2, search);
                pstat.setString(3, search);
                resset = pstat.executeQuery();
