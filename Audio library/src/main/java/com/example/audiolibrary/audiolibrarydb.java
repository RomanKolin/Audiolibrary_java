...
public class audiolibrarydb
{
    ...
    public static String[][] SELECT(int tabl) throws Exception
    {
        ...
        switch (tabl)
        {
            ...
            case 7:
                ...
                resset = stat.executeQuery("SELECT movanimsergam, artband, song, nosongs, REGEXP_REPLACE(TRIM(LEADING '0' FROM TRIM(LEADING '00:' FROM REPLACE(songsdur, ':0', ':'))), '^:', '0:'), ID FROM Soundtrack ORDER BY movanimsergam;");
                count = 0;
                while (resset.next())
                {
            ...
            case 8:
                ...
                resset = stat.executeQuery("WITH favourites AS(WITH favourites AS(SELECT `Music artist/band`.artband, `Related music artist/band`.relartband, `Music artist/band`.genr, Song.nam, `Cover's original music artist/band`.artband AS coverorigartbandID, `Cover's original music artist/band`.feat, `Music artist/band`.nosongs, `Music artist/band`.songsdur, `Music artist/band`.totnosongs, `Music artist/band`.totsongsdur, Favourites.num FROM `Music artist/band` JOIN Favourites ON `Music artist/band`.ID=Favourites.artband LEFT JOIN `Related music artist/band` ON `Music artist/band`.ID=`Related music artist/band`.artband JOIN `Music artist/band's song` ON `Music artist/band`.ID=`Music artist/band's song`.artband JOIN Song ON `Music artist/band's song`.song=Song.ID LEFT JOIN `Cover's original music artist/band` ON Song.ID=`Cover's original music artist/band`.song WHERE `Music artist/band`.cat='Жанр') SELECT favourites.artband, GROUP_CONCAT(DISTINCT relartband SEPARATOR ', ') AS relartband, favourites.genr, nam, `Music artist/band`.artband AS coverorigartband, feat, favourites.nosongs, favourites.songsdur, favourites.totnosongs, favourites.totsongsdur, num FROM favourites LEFT JOIN `Music artist/band` ON `Music artist/band`.ID=favourites.coverorigartbandID GROUP BY artband, genr, nam, coverorigartband, feat, nosongs, songsdur, totnosongs, totsongsdur, num) SELECT artband, relartband, genr, REPLACE(REPLACE(GROUP_CONCAT(CONCAT(IFNULL(nam, ''), ' (', IFNULL(coverorigartband, ''), ' & ', IFNULL(feat, ''), ')') ORDER BY coverorigartband IS NOT NULL, nam SEPARATOR ', '), ' ( & )', ''), ' & )', ')'), nosongs, TRIM(LEADING '0' FROM TRIM(LEADING '00:' FROM REPLACE(songsdur, ':0', ':'))), totnosongs, TRIM(LEADING '0' FROM TRIM(LEADING '00:' FROM REPLACE(totsongsdur, ':0', ':'))), num FROM favourites GROUP BY artband, relartband, genr, nosongs, songsdur, totnosongs, totsongsdur, num ORDER BY num;");
                count = 0;
                while (resset.next())
                {
                    String artband = resset.getString(1);
    ...
    public static String[][] SUB1SELECT(int ID) throws Exception
    {
        ...
        while (resset.next())
            count = resset.getInt("COUNT(ID)");
        datarr = new String[count][5];
        pstat = conn.prepareStatement("WITH relmusicartistband AS(WITH relmusicartistband AS(SELECT `Related music artist/band`.relartband, Song.nam, `Cover's original music artist/band`.artband AS coverorigartbandID, `Cover's original music artist/band`.feat, `Related music artist/band`.nosongs, `Related music artist/band`.songsdur, `Related music artist/band`.ID FROM `Music artist/band` JOIN `Related music artist/band` ON `Music artist/band`.ID=`Related music artist/band`.artband LEFT JOIN `Music artist/band's song` ON `Related music artist/band`.ID=`Music artist/band's song`.artband LEFT JOIN Song ON `Music artist/band's song`.song=Song.ID LEFT JOIN `Cover's original music artist/band` ON Song.ID=`Cover's original music artist/band`.song WHERE `Music artist/band`.ID=?) SELECT relartband, nam, `Music artist/band`.artband AS coverorigartband, feat, relmusicartistband.nosongs, relmusicartistband.songsdur, relmusicartistband.ID FROM relmusicartistband LEFT JOIN `Music artist/band` ON `Music artist/band`.ID=relmusicartistband.coverorigartbandID GROUP BY relartband, nam, coverorigartband, feat, nosongs, songsdur, ID ORDER BY relartband) SELECT relartband, REPLACE(REPLACE(GROUP_CONCAT(CONCAT(IFNULL(nam, ''), ' (', IFNULL(coverorigartband, ''), ' & ', IFNULL(feat, ''), ')') ORDER BY coverorigartband IS NOT NULL, nam SEPARATOR ', '), ' ( & )', ''), ' & )', ')'), nosongs, REGEXP_REPLACE(TRIM(LEADING '0' FROM TRIM(LEADING '00:' FROM REPLACE(songsdur, ':0', ':'))), '^:', '0:'), ID FROM relmusicartistband GROUP BY relartband, nosongs, songsdur, ID ORDER BY relartband;");
        pstat.setInt(1, ID);
        resset = pstat.executeQuery();
        count = 0;
        while (resset.next())
    ...
    public static String[][] SELECTSEARCHING(int tabl, int obslist, String search) throws Exception
    {
        ...
        switch (tabl)
        {
            case 3:
                if (obslist == 0)
                {
                    pstat = conn.prepareStatement("WITH musicartistband AS(WITH musicartistband AS((WITH musicartistband AS(WITH musicartistband AS(SELECT `Music artist/band`.artband, `Related music artist/band`.relartband, `Music artist/band`.genr, Song.nam, `Cover's original music artist/band`.artband AS coverorigartbandID, `Cover's original music artist/band`.feat, `Music artist/band`.nosongs, `Music artist/band`.songsdur, `Music artist/band`.totnosongs, `Music artist/band`.totsongsdur, `Music artist/band`.ID, `Related music artist/band`.ID AS relartbandid FROM `Music artist/band` LEFT JOIN `Related music artist/band` ON `Music artist/band`.ID=`Related music artist/band`.artband LEFT JOIN `Music artist/band's song` ON `Music artist/band`.ID=`Music artist/band's song`.artband LEFT JOIN Song ON `Music artist/band's song`.song=Song.ID LEFT JOIN `Cover's original music artist/band` ON Song.ID=`Cover's original music artist/band`.song WHERE `Music artist/band`.cat='Жанр' AND `Music artist/band`.artband LIKE ?) SELECT musicartistband.artband, GROUP_CONCAT(DISTINCT relartband ORDER BY relartbandid SEPARATOR ', ') AS relartband, musicartistband.genr, nam, `Music artist/band`.artband AS coverorigartband, feat, musicartistband.nosongs, musicartistband.songsdur, musicartistband.totnosongs, musicartistband.totsongsdur, musicartistband.ID FROM musicartistband LEFT JOIN `Music artist/band` ON `Music artist/band`.ID=musicartistband.coverorigartbandID GROUP BY artband, genr, nam, coverorigartband, feat, nosongs, songsdur, totnosongs, totsongsdur, ID) SELECT artband, relartband, genr, REPLACE(REPLACE(GROUP_CONCAT(CONCAT(IFNULL(nam, ''), ' (', IFNULL(coverorigartband, ''), ' & ', IFNULL(feat, ''), ')') ORDER BY coverorigartband IS NOT NULL, nam SEPARATOR ', '), ' ( & )', ''), ' & )', ')') AS nam, nosongs, REGEXP_REPLACE(TRIM(LEADING '0' FROM TRIM(LEADING '00:' FROM REPLACE(songsdur, ':0', ':'))), '^:', '0:') AS songsdur, totnosongs, REGEXP_REPLACE(TRIM(LEADING '0' FROM TRIM(LEADING '00:' FROM REPLACE(totsongsdur, ':0', ':'))), '^:', '0:') AS totsongsdur, ID FROM musicartistband GROUP BY artband, relartband, genr, nosongs, songsdur, totnosongs, totsongsdur, ID ORDER BY artband) UNION (WITH musicartistband AS(WITH musicartistband AS(SELECT `Music artist/band`.artband, `Related music artist/band`.relartband, `Music artist/band`.genr, Song.nam, `Cover's original music artist/band`.artband AS coverorigartbandID, `Cover's original music artist/band`.feat, `Music artist/band`.nosongs, `Music artist/band`.songsdur, `Music artist/band`.totnosongs, `Music artist/band`.totsongsdur, `Music artist/band`.ID, `Related music artist/band`.ID AS relartbandid FROM `Music artist/band` LEFT JOIN `Related music artist/band` ON `Music artist/band`.ID=`Related music artist/band`.artband LEFT JOIN `Music artist/band's song` ON `Music artist/band`.ID=`Music artist/band's song`.artband LEFT JOIN Song ON `Music artist/band's song`.song=Song.ID LEFT JOIN `Cover's original music artist/band` ON Song.ID=`Cover's original music artist/band`.song WHERE `Music artist/band`.cat='Жанр' AND `Related music artist/band`.relartband LIKE ?) SELECT musicartistband.artband, GROUP_CONCAT(DISTINCT relartband ORDER BY relartbandid SEPARATOR ', ') AS relartband, musicartistband.genr, nam, `Music artist/band`.artband AS coverorigartband, feat, musicartistband.nosongs, musicartistband.songsdur, musicartistband.totnosongs, musicartistband.totsongsdur, musicartistband.ID FROM musicartistband LEFT JOIN `Music artist/band` ON `Music artist/band`.ID=musicartistband.coverorigartbandID GROUP BY artband, genr, nam, coverorigartband, feat, nosongs, songsdur, totnosongs, totsongsdur, ID) SELECT artband, relartband, genr, REPLACE(REPLACE(GROUP_CONCAT(CONCAT(IFNULL(nam, ''), ' (', IFNULL(coverorigartband, ''), ' & ', IFNULL(feat, ''), ')') ORDER BY coverorigartband IS NOT NULL, nam SEPARATOR ', '), ' ( & )', ''), ' & )', ')') AS nam, nosongs, TRIM(LEADING '0' FROM TRIM(LEADING '00:' FROM REPLACE(songsdur, ':0', ':'))) AS songsdur, totnosongs, TRIM(LEADING '0' FROM TRIM(LEADING '00:' FROM REPLACE(totsongsdur, ':0', ':'))) AS totsongsdur, ID FROM musicartistband GROUP BY artband, relartband, genr, nosongs, songsdur, totnosongs, totsongsdur, ID ORDER BY artband)) SELECT artband, GROUP_CONCAT(relartband SEPARATOR ','), genr, nam, nosongs, songsdur, totnosongs, totsongsdur, ID FROM musicartistband GROUP BY artband, genr, nam, nosongs, songsdur, totnosongs, totsongsdur, ID ORDER BY artband) SELECT COUNT(ID) FROM musicartistband;");
            ...
            case 6:
                ...
                break;
            case 7:
                pstat = conn.prepareStatement("WITH soundtrack AS(WITH soundtrack AS((SELECT movanimsergam, artband, song, nosongs, REGEXP_REPLACE(TRIM(LEADING '0' FROM TRIM(LEADING '00:' FROM REPLACE(songsdur, ':0', ':'))), '^:', '0:') AS songsdur, ID FROM Soundtrack WHERE movanimsergam LIKE ? ORDER BY movanimsergam) UNION (SELECT movanimsergam, artband, song, nosongs, REGEXP_REPLACE(TRIM(LEADING '0' FROM TRIM(LEADING '00:' FROM REPLACE(songsdur, ':0', ':'))), '^:', '0:') AS songsdur, ID FROM Soundtrack WHERE artband LIKE ? ORDER BY movanimsergam) UNION (SELECT movanimsergam, artband, song, nosongs, REGEXP_REPLACE(TRIM(LEADING '0' FROM TRIM(LEADING '00:' FROM REPLACE(songsdur, ':0', ':'))), '^:', '0:') AS songsdur, ID FROM Soundtrack WHERE song LIKE ? ORDER BY movanimsergam)) SELECT movanimsergam, artband, song, nosongs, songsdur, ID FROM soundtrack ORDER BY movanimsergam) SELECT COUNT(ID) FROM soundtrack;");
                pstat.setString(1, search);
                pstat.setString(2, search);
                pstat.setString(3, search);
                resset = pstat.executeQuery();
